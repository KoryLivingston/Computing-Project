...
Public Class CitadelClash
    ...
    Private TotalCoinsEarned() As Integer = {"741", "483", "351", "675", "145", Nothing}
    ...
    'ATTTRIBUTES FOR INTEGERATED DBDD
    ...
    Public Sub New()
        ...
        DBFileName = CurDir() & "\Databases\Game.accdb"
        createGameDatabase()

        connection = New OleDbConnection(connectionType & "Data Source=" & DBFileName)
    ...
    Private Sub newName_KeyDown(sender As Object, e As KeyEventArgs) Handles newName.KeyDown
        ...
        If e.KeyCode = System.Windows.Forms.Keys.Enter And newName.Text.Length = 3 Then
    ...
    'Function provided by teacher, to replace the functionality of IF NOT EXISTS
    ...
    Public Function createGameDatabase() As Boolean

        Dim DBcreated As Boolean = True

        'Creates Game database

        Dim catalog As New Catalog()

        Try
            Dim createDB As String
            createDB = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" & DBFileName
            catalog.Create(createDB)
